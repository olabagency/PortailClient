          ...
          <ul className="space-y-3">
            {features.map((f) => (
              <li key={f.title} className="flex items-start gap-2.5">
                <span className="mt-0.5 flex-shrink-0 rounded-full bg-[#386FA4]/20 p-1">
                  <CheckCircle2 className="h-3.5 w-3.5 text-[#386FA4]" />
                </span>
                <div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className="h-3 w-3 fill-[#386FA4]" viewBox="0 0 20 20" aria-hidden="true">
